...
export default function NotificationSystem({ user, socket, panelOpen: controlledPanel, onTogglePanel, hideBell }) {
  ...
  useEffect(() => {
    ...
    const handler = (notif) => {
      ...
      if (notif.jobId) {
        window.dispatchEvent(new CustomEvent('gshop:refresh-jobs', {
    ...
    const nearbyJobHandler = (data) => {
      ...
      timeoutsRef.current.push(timer);
      // Refresh jobs list
      window.dispatchEvent(new CustomEvent('gshop:refresh-jobs', { detail: { type: 'nearby_job_posted' } }));
    };
    ...
    const nearbyServiceHandler = (data) => {
      ...
      window.dispatchEvent(new CustomEvent('gshop:refresh-services', { detail: { type: 'nearby_service_posted' } }));
    };
