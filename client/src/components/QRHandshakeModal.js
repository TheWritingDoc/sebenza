...
function QRHandshakeModal({ jobId, userId, isPoster, onClose, onScanned, handshakeMode = 'start', job }) {
  ...
  const handshakePayload = `gshop-handshake|${jobId}|${userId}|${handshakeMode}`;
  ...
  useEffect(() => {
    ...
    const onHandshakeComplete = (data) => {
      ...
      setResult({
        phase: 'show_scanned',
        icon: '🎉',
        title: 'You were scanned!',
        message: 'The job has been started.',
        subtext: 'Both of you have confirmed. You can close this window.',
        color: '#166534',
        bg: '#f0fdf4',
        border: '#bbf7d0',
      });
  ...
  const handleScan = async (decodedText) => {
    ...
    if (parts.length < 3 || parts[0] !== 'gshop-handshake') {
              ...
              <>
                ...
                {mode === 'scan' && (
                  <div style={{ textAlign: 'center' }}>
                    ...
                    {debugInfo && (
                      <div style={{ marginTop: 8, padding: 8, borderRadius: 8, background: '#f1f5f9', color: '#475569', fontSize: 10, fontFamily: 'monospace' }}>
                        {debugInfo}
                      </div>
                    )}
                    <button
                      onClick={async () => {
                        try {
                          const testUrl = `${API_URL}/api/jobs/${String(jobId)}/qr-test`;
                          setScanError('');
                          setDebugInfo(`Testing: ${testUrl}`);
                          const res = await fetch(testUrl, { headers: { Authorization: `Bearer ${localStorage.getItem('token')}` } });
                          const contentType = res.headers.get('content-type') || '';
                          if (contentType.includes('text/html')) {
                            const text = await res.text();
                            setDebugInfo(`Test FAILED: Server returned HTML (length ${text.length}). Wrong port? Use localhost:3001.`);
                            return;
                          }
                          const data = await res.json();
                          setDebugInfo(`Test OK: ${JSON.stringify(data)}`);
                        } catch (e) {
                          setDebugInfo(`Test FAILED: ${e.message}`);
                        }
                      }}
                      style={{ marginTop: 12, padding: '8px 16px', borderRadius: 8, border: '1px solid #cbd5e1', background: '#f8fafc', color: '#475569', fontSize: 12, fontWeight: 600, cursor: 'pointer' }}
                    >
                      🧪 Test API Route
                    </button>
                  </div>
