...
function JobReviewModal({ job, user, onClose, onSubmitted }) {
  ...
  const isPoster = job.posterId?._id?.toString?.() === userIdStr || job.posterId?.toString?.() === userIdStr;

  const app = job.applications?.find(a => a._id?.toString?.() === job.acceptedApplicationId?.toString?.());
  const revieweeName = isPoster
    ? (app?.applicantId?.name || 'Helper')
    : (job.posterId?.name || 'Neighbour');
  ...
  const handleSubmit = async () => {
    if (needsConstructive && comment.trim().length < 10) {
      setMsg('Please share at least 10 characters of constructive feedback to help them grow.');
      return;
    }
    setLoading(true);
    setMsg('');
    try {
      ...
      setMsg(err.response?.data?.error || err.response?.data?.details || 'Failed to submit review.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const renderStars = (cat, value) => (
    <div style={{ display: 'flex', gap: 6, justifyContent: 'center' }}>
      {[1, 2, 3, 4, 5].map(s => (
        <button key={s} onClick={() => handleCategoryClick(cat, s)} style={{
          fontSize: 28, background: 'none', border: 'none', cursor: 'pointer',
          opacity: s <= value ? 1 : 0.25, transform: s <= value ? 'scale(1.1)' : 'scale(1)',
  ...
  return (
    <div style={modalOverlayStyle}>
      <div style={{ ...modalContentStyle(440) }}>
        ...
        {msg && (
          <div style={{
            background: msg.includes('Failed') ? '#fef2f2' : '#fef9c3',
            color: msg.includes('Failed') ? '#991b1b' : '#854d0e',
        ...
        {step === 1 && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
            ...
            <div style={{ background: '#f0fdf4', borderRadius: 16, padding: 14, textAlign: 'center', marginTop: 4 }}>
              <div style={{ fontSize: 13, color: '#166534', fontWeight: 600 }}>Overall: {overallRating}/5</div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            <div>
              <label style={{ fontSize: 14, fontWeight: 700, color: '#374151', display: 'block', marginBottom: 6 }}>
                {needsConstructive ? '💚 Help them grow — what could be better?' : '✨ What did they do well? (optional)'}
              </label>
            ...
            <div style={{ background: '#f8fafc', borderRadius: 16, padding: 14 }}>
              <div style={{ fontSize: 12, fontWeight: 700, color: '#64748b', marginBottom: 8, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Your ratings</div>
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
                {Object.entries(categories).map(([key, val]) => (
                  ...
                  }}>{CATEGORY_LABELS[key].split(' ').slice(1).join(' ')}: {val}★</span>
                ))}
              </div>
            </div>
